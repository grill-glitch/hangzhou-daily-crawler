...
#### `archive` - **查找 Wayback Machine 存档** 🆕
当网页抓取失败时，可从 Wayback Machine 获取历史存档。

```bash
# 查询某 URL 的可用存档（返回存档列表）
python3 skills/internet-archive/internet-archive.py archive https://example.com

# 限制年份范围
python3 skills/internet-archive/internet-archive.py archive https://example.com --from=2006 --to=2010

# 限制返回数量
python3 skills/internet-archive/internet-archive.py archive https://example.com --limit=5
...
**输出特点：**
- 自动显示存档的时间戳（如 `"archive_date": "2007-01-11 15:33:18"`）
- 返回存档的完整 URL，可直接访问
- 列出多个可用存档供选择
- `--fetch` 模式下返回页面内容（限制 5000 字符）

**典型 use case：**
当 `web_fetch` 遇到 404 或其他错误时，可调用此 intent 获取历史版本，并提醒用户存档时间。

```bash
# 示例：获取 2007 年初代 iPhone 页面
python3 skills/internet-archive/internet-archive.py archive https://www.apple.com/iphone/ --fetch
```
